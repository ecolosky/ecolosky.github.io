...
content += '<masonry column-width=1>';
content += '<masonry-brick ng-repeat="class in view" class="bookmarklet label {{class.color}}" style="{{class.size}}">{{class.name}}</masonry-brick>';
content += '</div>';
content += '</masonry>';
content += '</div>';
content += '</div>';
...
var app = angular.module('bookmarklet', ['wu.masonry']);
